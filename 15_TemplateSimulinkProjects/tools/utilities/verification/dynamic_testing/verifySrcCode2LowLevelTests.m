function verifySrcCode2LowLevelTests(modelName, varargin)
%verifySrcCode2LowLevelTests Verify source code against low-level tests
...
%   the compiled code on the host computer via SIL simulations.
%
%   verifySrcCode2LowLevelTests(ModelName)
%   verifySrcCode2LowLevelTests(ModelName, AuthorNames)
...
testDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'executable_tests', modelName, 'host', 'low_level_tests');
covDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'code_coverages', modelName, 'low_level_tests');
resultFile = fullfile(testDir, [modelName, '_SIL_SLDV_Based_Test_Results.mldatx']);
rptFile = fullfile(testDir, [modelName, '_SIL_SLDV_Based_Test_Report.pdf']);
cvtFile = fullfile(covDir, [modelName '_SIL_SLDV_Based_Code_Coverage.cvt']);
htmlFile = fullfile(covDir, [modelName '_SIL_SLDV_Based_Code_Coverage_Report.html']);
...
disp(['Running low-level SIL tests on source code of Simulink model ', modelName, '.']);
...
sltest.testmanager.report(testResult, rptFile, ...
    'Author', authors, ...
    'Title',[modelName, ' SIL Low-Level Tests'], ...
