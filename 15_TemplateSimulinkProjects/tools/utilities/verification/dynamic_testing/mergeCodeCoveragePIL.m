function mergeCodeCoveragePIL(modelName)
%mergeCodeCoverage Merge coverage results of source code
...
testDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'executable_tests', modelName, 'target');
covDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'code_coverages', modelName);
reqRslt = fullfile(testDir, 'high_level_tests', [modelName, '_PIL_REQ_Based_Test_Results.mldatx']);
sldvRslt = fullfile(testDir, 'low_level_tests', [modelName, '_PIL_SLDV_Based_Test_Results.mldatx']);

% Delete the old results and reports if they exist.
htmlFile = fullfile(covDir, [modelName '_PIL_Merged_Code_Coverage_Report.html']);
...
if ~exist(reqRslt, 'file')
    error(['Test results ''', modelName, '_PIL_REQ_Based_Test_Results.mldatx'' not found.']);
end
if ~exist(sldvRslt, 'file')
    error(['Test results ''', modelName, '_PIL_SLDV_Based_Test_Results.mldatx'' not found.']);
end
