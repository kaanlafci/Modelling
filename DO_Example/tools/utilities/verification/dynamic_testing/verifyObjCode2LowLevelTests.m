function verifyObjCode2LowLevelTests(modelName, varargin)
%verifyObjCode2LowLevelTests Verify executable object code against
%low-level tests
%   Verify the target code against low-level tests generated by Design
%   Verifier. All tests exercise the compiled code on the target computer
%   via PIL simulations.
%
%   verifyObjCode2LowLevelTests(ModelName)
%   verifyObjCode2LowLevelTests(ModelName, AuthorNames)
...
testDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'executable_tests', modelName, 'target', 'low_level_tests');
covDir = fullfile(Simulink.fileGenControl('get', 'CodeGenFolder'), '..', 'verification', 'code_coverages', modelName, 'low_level_tests');
resultFile = fullfile(testDir, [modelName, '_PIL_SLDV_Based_Test_Results.mldatx']);
rptFile = fullfile(testDir, [modelName, '_PIL_SLDV_Based_Test_Report.pdf']);
cvtFile = fullfile(covDir, [modelName '_PIL_SLDV_Based_Code_Coverage.cvt']);
htmlFile = fullfile(covDir, [modelName '_PIL_SLDV_Based_Code_Coverage_Report.html']);
...
if ~exist([modelName, '_SLDV_Based_Test.mldatx'], 'file')
    error(['Test file ''', modelName, '_SLDV_Based_Test.mldatx'' not found.']);
end
...
% Verify the target code against LLR test cases in the test file without coverage analysis.
disp(['Running low-level PIL tests on source code of Simulink model ', modelName, '.']);
sltest.testmanager.clear();
sltest.testmanager.clearResults();
testFile = sltest.testmanager.load([modelName, '_SLDV_Based_Test.mldatx']);
...
sltest.testmanager.report(testResult, rptFile, ...
    'Author', authors, ...
    'Title',[modelName, ' PIL Low-Level Tests'], ...
